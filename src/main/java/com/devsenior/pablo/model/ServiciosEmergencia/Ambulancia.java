 ...
 * @author Juanpa
 */
public class Ambulancia extends IServiceEmergencia {

    public Ambulancia(String id, Integer personal) {
        super(id, personal);
    }

    @Override
    public void atenderEmergencia(Emergencia emeEnergencia) {
        System.out.println("Ya enviamos los paramedicos!");
        System.out.println("Cuanto personal envia para esta emergencia?");
        Integer cantidad = entrada.nextInt();
        Boolean estado = false;
        while (!estado) {
            estado = this.enviarPersonal(cantidad);
        }
        estado = false;
        while (!estado) {
            estado = this.enviarVehiculo();
        }
